...
class InferenceService:
    ...
    # 通用节点推理：输出预测标签与置信度
    def _predict_node(self, model: Any, input_ids: Any, attention_mask: Any) -> tuple[int, float]:
        outputs = model(input_ids=input_ids, attention_mask=attention_mask)
        probs = self._nnf.softmax(outputs.logits, dim=-1)
    ...
    def evaluate(self, question: str, answer: str) -> dict[str, Any]:
        ...
        extracted_entities = self.keyword_processor.extract_keywords(combined_text)
        # 前置门卫：长度过短或无金融实体时直接拦截
        if len(combined_text) < self.settings.entity_min_length or len(extracted_entities) == 0:
            return {
                "root_id": -1,
