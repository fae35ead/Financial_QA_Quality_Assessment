...
def build_financial_processor():
    ...
    dict_files = [
        PROJECT_ROOT / "data" / "others" / "THUOCL_caijing.txt",
        PROJECT_ROOT / "data" / "others" / "baostock_entities.txt",
        PROJECT_ROOT / "data" / "others" / "custom_entities.txt",
    ]
    ...
    for file_path in dict_files:
        try:
            ...
            print(f"警告: 找不到实体词典文件 {str(file_path)}")
...
def main():
    ...
    output_path = PROJECT_ROOT / "data" / "processed" / "stage1_filtered_data.csv"
    rejected_output_path = PROJECT_ROOT / "data" / "processed" / "stage1_rejected_data.csv"
