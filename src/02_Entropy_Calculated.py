'''该文件负责计算每条问答对的熵值（信息增益），并将结果保存到新的 CSV 文件中。主要步骤包括：
1. 读取预处理后的数据。
2. 对 'Qsubj' 和 'Reply' 列进行中文分词，并过滤掉无意义的 token（如纯标点、特殊字符、数字等）。
3. 计算每条问答对的长度惩罚分数，根据回复长度与问题长度的比例进行调整。
4. 计算每条问答对的新词比例（信息增益），即回复中不在问题中的新词占回复总词数的比例。
5. 计算每条问答对的语义相似度，使用 TF-IDF 向量化后计算余弦相似度。
6. 综合长度惩罚分数、新词比例和语义相似度，计算最终的得分，并将结果保存到新的 CSV 文件中。
'''
...
INPUT_PATH = PROJECT_ROOT / "data" / "processed" / "stage1_filtered_data.csv"
OUTPUT_PATH = PROJECT_ROOT / "data" / "processed" / "stage2_entropy_calculated_data.csv"
