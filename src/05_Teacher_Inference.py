...
UNLABELED_DATA_FILE = PROJECT_ROOT / "data" / "processed" / "stage2_entropy_calculated_data.csv"
OUTPUT_FILE = PROJECT_ROOT / "data" / "processed" / "soft_labeled_100k.csv"
TEACHER_MODEL_DIR = PROJECT_ROOT / "models" / "teacher_cross_encoder"
...
def main():
    ...
    sample_n = min(SAMPLE_SIZE, len(df_all))
    if sample_n <= 0:
        raise ValueError(f"数据文件为空，无法抽样: {UNLABELED_DATA_FILE}")

    print(f"原始数据总量: {len(df_all)} 条。开始随机抽取 {sample_n} 条...")
    df_sample = df_all.sample(n=sample_n, random_state=42).copy()
